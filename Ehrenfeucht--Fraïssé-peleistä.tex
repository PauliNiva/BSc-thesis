...
\begin{esim}
Oletetaan että ensimmäisen kertaluokan predikaattilogiikan lause $\varphi$ määrittelee verkkojen yhtenäisyyden aakkostossa joka muodostuu binaarisesta relaatiosymbolista $E$.
Määritellään seuraajarelaatio $S$ seuraavasti: \[S(x, y) \coloneqq (x < y) \land \forall z ((z \leq x) \lor (y \leq z))\]
...
\end{itemize}
Ensimmäisen kertaluokan predikaattilogiikan kaavan $\psi$ määräämä suunnattu verkko $G$ on yhtenäinen, tarkemmin sanoen se muodostuu yhdestä syklistä, jos sen solmujen määrä on pariton.
...
{Verkko jossa pariton määrä solmuja};
\end{tikzpicture}
\end{center}

Jos solmujen määrä on parillinen niin verkko ei ole yhtenäinen, tarkemmin sanoen se koostuu kahdesta erillisestä syklistä.
...
{Verkko jossa parillinen määrä solmuja};
\end{tikzpicture}
\end{center}
